...
pg.display.set_caption("Catch the bubble")
background = pg.image.load(background_image_filename).convert()
...
num = 5
bubble_img_filename = 'Untitled-1.png'
bubls = pg.sprite.Group()
bublers = []
for i in range(num):
    bsize = random.randrange(20, 70)
    bubblex = round(random.randrange(bsize, dis_width - bsize) / 10.0) * 10.0
    bubbley = round(random.randrange(0, dis_height - 430) / 10.0) * 10.0
    bublers.append(Bubble(bubblex,bubbley, bubls, bsize))
# for i in range(3):
#     bubblex = round(random.randrange(bubble_size, dis_width - bubble_size) / 10.0) * 10.0
#     bubbley = round(random.randrange(0, dis_height - 430) / 10.0) * 10.0
#     bub = Bubble(bubblex,bubbley, bubble_img_filename)
#     bubls.add(bub)
...
while 1:
    ...
    screen.fill(WHITE)
    bubls.draw(screen)
    # screen.blit(bub1.image, bub1.rect)
    # screen.blit(bub2.image, bub2.rect)
    # screen.blit(bub3.image, bub3.rect)
    pg.display.update()
    pg.time.delay(20)

    # машинка ездит сверху вниз
    # bub.speed()
    bubls.update(dis_height)
